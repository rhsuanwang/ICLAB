...
//---------------------------------------------------------------------
wire sleep_in,sleep_inv,sleep_mul,sleep_sort,sleep_sum,sleep_out;
wire clk_in,clk_inv,clk_mul,clk_sort,clk_sum,clk_out;
GATED_OR GATED_IN(.CLOCK(clk),
                  .SLEEP_CTRL(cg_en & sleep_in),
                  .RST_N(rst_n),
                  .CLOCK_GATED(clk_in));
GATED_OR GATED_INV(.CLOCK(clk),
                   .SLEEP_CTRL(cg_en & sleep_inv),
                   .RST_N(rst_n),
                   .CLOCK_GATED(clk_inv));
GATED_OR GATED_MUL(.CLOCK(clk),
                   .SLEEP_CTRL(cg_en & sleep_mul),
                   .RST_N(rst_n),
                   .CLOCK_GATED(clk_mul));                    
GATED_OR GATED_SORT(.CLOCK(clk),
                    .SLEEP_CTRL(cg_en & sleep_sort),
                    .RST_N(rst_n),
                    .CLOCK_GATED(clk_sort));
GATED_OR GATED_SUM(.CLOCK(clk),
                   .SLEEP_CTRL(cg_en & sleep_sum),
                   .RST_N(rst_n),
                   .CLOCK_GATED(clk_sum));     
GATED_OR GATED_OUT(.CLOCK(clk),
                   .SLEEP_CTRL(cg_en & sleep_out),
                   .RST_N(rst_n),
                   .CLOCK_GATED(clk_out));          
assign sleep_in = (state == IDLE || state == IN) ? 1'b0:1'b1;
assign sleep_inv = (state == INV) ? 1'b0:1'b1;
assign sleep_mul = (state == MUL) ? 1'b0:1'b1;
assign sleep_sort= (state == SORT)? 1'b0:1'b1;
assign sleep_sum = (state == SUM) ? 1'b0:1'b1;
assign sleep_out = (state == OUT || state == IDLE) ? 1'b0:1'b1;
...
//---------------------------------------------------------------------
always @(posedge clk_sum or negedge rst_n) begin
    if(!rst_n) begin
        e[0] <= 0;
        e[1] <= 0;
        e[2] <= 0;
